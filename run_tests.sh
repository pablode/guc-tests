...
# UsdaDiff
if ! skip_or_print_test "UsdaDiff_Below2502"; then
    USD_OUTPUT_FILE="output/UsdaDiff_Below2502.usda"
    ...
    if [ $? -ne 0 ] || ! diff --text -w "$USD_OUTPUT_FILE" "UsdaDiff_Below2502.usda"; then
        print_error
    fi
fi
if ! skip_or_print_test "UsdaDiff_Equal2502"; then
    USD_OUTPUT_FILE="output/UsdaDiff_Equal2502.usda"
    convert_glTF_to_USD "input/glTF-Asset-Generator/Output/Positive/Material_MetallicRoughness/Material_MetallicRoughness_11.gltf" $USD_OUTPUT_FILE -m -u

    if [ $? -ne 0 ] || ! diff --text -w "$USD_OUTPUT_FILE" "UsdaDiff_Equal2502.usda"; then
        print_error
    fi
fi
if ! skip_or_print_test "UsdaDiff_Above2502"; then
    USD_OUTPUT_FILE="output/UsdaDiff_Above2502.usda"
    convert_glTF_to_USD "input/glTF-Asset-Generator/Output/Positive/Material_MetallicRoughness/Material_MetallicRoughness_11.gltf" $USD_OUTPUT_FILE -m -u

    if [ $? -ne 0 ] || ! diff --text -w "$USD_OUTPUT_FILE" "UsdaDiff_Above2502.usda"; then
        print_error
    fi
fi
